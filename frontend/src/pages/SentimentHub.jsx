...
const SentimentHub = () => {
  ...
  }, [selectedTicker, fetchSentiment]);

  const loadRegime = useCallback(async () => {
    const data = await fetchMarketRegime();
    if (data?.success) setMarketRegime(data.data);
  }, [fetchMarketRegime]);

  useEffect(() => { loadSentiment(); loadRegime(); }, [loadSentiment, loadRegime]);
            ...
            {sentiment && (
              <div>
                <div className="flex items-center gap-4 mb-4 pb-4 border-b border-[#1F2937]">
                  ...
                  <div><span className="text-[10px] font-mono uppercase text-[#6B7280]">Score</span><div className="mt-1 text-lg font-mono font-medium text-[#F9FAFB]">{sentiment.overall_sentiment?.toFixed(2)}</div></div>
                  <div><span className="text-[10px] font-mono uppercase text-[#6B7280]">News Count</span><div className="mt-1 text-lg font-mono font-medium text-[#F9FAFB]">{sentiment.news_count}</div></div>
                </div>
                <div className="space-y-3">
                  <h3 className="text-base font-medium text-[#F9FAFB]">Recent Headlines</h3>
                  {sentiment.headlines?.map((news) => (
                    <div key={`${news.title}-${news.source}`} className="border-l-2 border-[#1F2937] pl-3 py-2 hover:border-[#3B82F6] transition-colors" data-testid={`news-${news.source}`}>
                      <div className="flex items-start gap-2">
                        {getSentimentIcon(news.sentiment)}
                        <div className="flex-1">
                          <h4 className="text-sm text-[#F9FAFB] mb-1">{news.title}</h4>
                          <div className="flex items-center gap-3 text-xs text-[#6B7280]">
                            <span>{news.source}</span><span>&bull;</span>
        ...
        <div className="space-y-4">
          <div className="bg-[#111827] border border-[#1F2937] rounded-sm p-4">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-[#9CA3AF]">Regime</span><span className="text-[#F9FAFB] font-mono text-xs">{marketRegime?.regime || '-'}</span></div>
              <div className="flex justify-between"><span className="text-[#9CA3AF]">VIX</span><span className="text-[#F9FAFB] font-mono">{marketRegime?.vix?.toFixed(2) || '-'}</span></div>
              <div className="flex justify-between"><span className="text-[#9CA3AF]">FII Flow</span><span className={`font-mono ${marketRegime?.fii_flow_5d >= 0 ? 'text-[#10B981]' : 'text-[#EF4444]'}`}>₹{marketRegime?.fii_flow_5d?.toFixed(0) || '0'} Cr</span></div>
            </div>
          </div>
          <div className="bg-[#111827] border border-[#1F2937] rounded-sm p-4">
            <h3 className="text-base font-medium text-[#F9FAFB] mb-3">Sentiment Guide</h3>
            <div className="space-y-2 text-xs text-[#9CA3AF]">
              <div className="flex items-start gap-2"><TrendingUp size={14} className="text-[#10B981] mt-0.5" /><span>Positive sentiment indicates bullish news flow</span></div>
              <div className="flex items-start gap-2"><TrendingDown size={14} className="text-[#EF4444] mt-0.5" /><span>Negative sentiment suggests bearish outlook</span></div>
              <div className="flex items-start gap-2"><Minus size={14} className="text-[#F59E0B] mt-0.5" /><span>Neutral sentiment shows mixed signals</span></div>
            </div>
          </div>
        </div>
      </div>
